...
public class ErlangProcessContext {
    ...
    public String stateRecordName;

    private static int whileNum = 0;

    public boolean internalContext = false;
    public boolean sensitiveContext = false;
    public boolean recursiveContext = false;
    public String internalFunctionName = "";
    public List<String> internalFunctionArgs = new Vector<>();

    public boolean skipStartFunctions = false;
    /**
     ...
     */
    public TreeMap<String, String> temporaryVarNames = new TreeMap<>(String.CASE_INSENSITIVE_ORDER);
    /**
    ...
    /** This map contains the map of functions that is filled while translating.
    Each of these functions cannot be translated due to special behaviour **/
    public HashMap<String, Integer> danglingFunctions = new HashMap<>();
    ...
    public ErlangProcessContext(ErlangProcessContext context, boolean isNewScope) {
        ...
        this.danglingFunctions = context.danglingFunctions;
    ...
    private String getVarAccess(String pcalVarName) {
        return formatVarAccess(getStateField(pcalVarName), false);
    }
    ...
    ErlangStateField getStateField(String pcalName) {
        return stateFieldNames.get(pcalName);
    }

    protected String formatVarAccess(ErlangStateField field) {
        return formatVarAccess(field, false);
    }

    protected String formatVarAccess(ErlangStateField field, boolean inPlace) {
        ...
        if (inPlace) {
            return String.format(PROCESS_FIELD_ACCESS_INPLACE, stateRecordName, field.getName());
        } else {
     ...
     */
    public String getErlangVarStr(String pcalVarName) {
        if (temporaryVarNames.containsKey(pcalVarName)) {
            ...
            if (isInRecordScope()) {
                 ...
                 */
                if (danglingFunctions.containsKey(pcalVarName)) {
                    return "";
                } else {
                    try {
                        stateFieldNames.put(pcalVarName, new ErlangStateField(pcalVarName, ErlangStateField.FieldType.CONSTANT));
    ...
    private void fillProcessLocalVarNames(Vector<AST.VarDecl> fieldVarDecls) throws PcalErlangGenException {
        for (AST.VarDecl varDeclObj : fieldVarDecls) {
            String pcalName = varDeclObj.var;
            stateFieldNames.put(pcalName, new ErlangStateField(pcalName, ErlangStateField.FieldType.PROCESS_LOCAL_VARIABLE));
        }
    }
