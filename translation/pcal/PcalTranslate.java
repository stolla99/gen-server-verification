...
//                ? st.UseThis(PcalSymTab.LABEL, "Done", "")
    ...
    private static AST.Multiprocess ExplodeMultiprocess (AST.Multiprocess ast) throws PcalTranslateException {
        ...
        *********************************************************************/
        ...
        newast.name = ast.name;
        newast.decls = ast.decls;
        newast.procedures = new Vector(ast.procedures.size(), 10);
        newast.defs = ast.defs ;  // added 25 Jan 2006 by LL
        newast.setOrigin(ast.getOrigin()) ;
        while (i < ast.procedures.size()) {
            newast.procedures.addElement(ExplodeProcedure(ast.procedures.elementAt(i)));
            i = i + 1;
        }
        i = 0;
        newast.processes = new Vector(ast.processes.size(), 10);
        while (i < ast.processes.size()) {
            newast.processes.addElement( ExplodeProcess(ast.processes.elementAt(i)));
            i = i + 1;
        }
        return newast;
    }
