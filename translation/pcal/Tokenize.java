...
public class Tokenize {
    ...
    public static Pair<TLAExpr, List<Pair<String, String>>> TokenizeExpr(PcalCharReader charReader) throws TokenizerException {
        TLAExpr exp = InnerTokenize(charReader, true);
        ArrayList<Pair<String, String>> optionals = new ArrayList<>();
        if (!exp.tokens.isEmpty()) {
            Vector<TLAToken> tlaTokens = (Vector<TLAToken>) exp.tokens.get(0);
            List<String> tokenStrings = new ArrayList<>(tlaTokens.stream().map(e -> e.string).toList());
            int aCount = tokenStrings.stream().filter(a -> a.equals("a")).toList().size();
            for (int i = 0; i < aCount; i++) {
                if (tokenStrings.contains("a")) {
                    int aIndex = tokenStrings.indexOf("a");
                    if (tlaTokens.size() >= aIndex + 1 && tlaTokens.get(aIndex + 1).type == TLAToken.STRING) { // it is a string
                        ((Vector<?>) exp.tokens.get(0)).remove(aIndex);
                        // System.out.println(" 🔍 Found atom " + String.join("", tokenStrings));
                        optionals.add(new Pair<>("§atom", tokenStrings.get(aIndex + 1)));
                    }
                    tokenStrings.remove(aIndex);
                }
            }
        }
        return new Pair<>(exp, optionals);
    }
