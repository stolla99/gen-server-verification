...
//                Map.entry("\\/", new MapValueTuple("%s or %s", 14, false)),
    ...
    public String TLAExprToErlangStrInternal(List<TLAToken> tokens, ErlangProcessContext context) throws PcalErlangGenException {
        if (tokens.isEmpty()) {
            return "";
        }
        ...
        if (firstOpPos < 0) {
            ...
            } else if (tokens.get(0).string.equals("(")) {
                int nextParenPos = -1;
                for (int i = 0; i < tokens.size(); i++) {
                    if (tokens.get(i).string.equals(")")) {
                        nextParenPos = i;
                    }
                }
                if (nextParenPos > 0 && tokens.get(nextParenPos).string.equals(")")) {
                    tokens.remove(0);
                    tokens.remove(nextParenPos-1);
                }
            }
            ...
            switch (firstToken.string) {
                ...
                case "<|":
                    return tupleGen(tokens, context);
            }
            ...
            if (tokenType == TLAToken.STRING) {
                ...
                if (Preprocessor.isTlaFunctionCall(tokens)) {
                    String fName = Preprocessor.getFunctionName(tokens);
                    Pair<String, String> pair = Preprocessor.getFunctionArgs(tokens, context, this);
                    ///  $replacement$ -> fname(replacement)
                    String substitution = pair.getFirst().substring(1, pair.getFirst().length() - 1);
                    String args = pair.getSecond();
                    if (!substitution.trim().isEmpty()) {
                        args = args.replace(substitution, UTIL_MODULE_NAME + ":" + fName + "(" + substitution + ")");
                        context.danglingFunctions.putIfAbsent(fName, args.split(",").length);
                        return args;
                    } else {
                        return UTIL_MODULE_NAME + ":" + fName + "(" + args + ")";
                    }
                } else if (StringUtils.decapitalize(pcalVarName).startsWith(PREFIX_RETURN_VALUE)) {
                    currTokenStr = context.temporaryVarNames.get(pcalVarName);
                    if (Preprocessor.isAccess(tokens)) {
                        return Preprocessor.getAccess(tokens, currTokenStr);
                    } else {
                        return currTokenStr;
                    }
                } else if (tokens.size() > 1 && tokens.get(1).string.equals("[")) {
                    context.getErlangVarStr(pcalVarName);
                    return genAccess(tokens, context);
                } else {
                    if (pcalVarName.equals("rootModule")) {
                        currTokenStr = "?MODULE";
                    } else {
            ...
            List<TLAToken> remaining = tokens.subList(1, tokens.size());
            if (!remaining.isEmpty()) {
                return currTokenStr + this.TLAExprToErlangStrInternal(remaining, context);
            } else {
                return currTokenStr;
            }
    ...
    public String parseAssignment(AST.Lhs lhs, TLAExpr rhs, ErlangProcessContext context) throws PcalErlangGenException {
        ...
        if (rhs.annotations.stream().map(Pair::getFirst).anyMatch(Preprocessor::isAtom)) {
            right = Preprocessor.transformAtom(right);
        }
    ...
    public Map.Entry<String, String> parseDefinition(Vector<TLAToken> def, ErlangDefinitionContext context) throws PcalErlangGenException {
        ...
        if (!def.get(1).string.equals("==")) {
            PcalDebug.reportInfo("      🔴 skipping definition \"" + pcalDefName + "\". Expected second token to be \"==\", but was " + def.get(1).string + ".");
            return null;
        }
    ...
    private String genAccess(List<TLAToken> tokens, ErlangProcessContext context) throws PcalErlangGenException {
        ...
        if (keys.split(",").length >= 2) {
            ...
            Iterator<String> iterator = Arrays.stream(keys.split(",")).map(String::trim).iterator();
            do {
                String next = iterator.next();
                if (isFirst && iterator.hasNext()) {
                    out = "util:tuple_to_list(" + String.format(NTH_ELEMENT, next, erlangVar) + ")";
                    isFirst = false;
                } else {
                    if (iterator.hasNext()) {
                        out = "util:tuple_to_list(" + String.format(NTH_ELEMENT, next, out) + ")";
                    } else {
                        out = String.format(NTH_ELEMENT, next, out);
                    }
                }
            } while (iterator.hasNext());
            return out;
        } else {
            return String.format(NTH_ELEMENT, keys, erlangVar);
        }
    ...
    private String tupleGen(List<TLAToken> tokens, ErlangProcessContext context) throws PcalErlangGenException {
        tokens.remove(0);
        tokens.remove(tokens.size() - 1);
        String[] parameters = parseArguments(tokens, context);
        // return result
    ...
    private String recordGen(List<TLAToken> tokens, ErlangProcessContext context) throws PcalErlangGenException {
        ...
        if (!context.sensitiveContext) {
            return String.format("#{%s}", argStr);
        } else {
            return String.format("#" + context.stateRecordName + "{%s}", argStr.replace("=>", "=")); // sensitive context
        }
    }
    ...
    private int findNextCommaPos(PCalTokenIterator iterator) {
        ...
        int skipUntilClosed = 0;
        while (currentToken != null) {
            if (currentToken.string.equals("<|")) {
                skipUntilClosed++;
            } else if (currentToken.string.equals("|>")) {
                skipUntilClosed--;
            }
            if (currentToken.string.equals(",") && skipUntilClosed == 0) {
