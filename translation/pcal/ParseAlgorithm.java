...
public class ParseAlgorithm {
    ...
    public static HashMap<String, Integer> macroExpandCount = new HashMap<>();

    public static LinkedList<String> macrosWithLabels = new LinkedList<>();
    public static LinkedList<String> dangerousLabels = new LinkedList<>();
    public static String currentMacroContext = "";

    public static Boolean internalVariables = false;
    ...
    public static HashMap<String, ArrayList<String>> macrosWithAnnotations = new HashMap<>();
    public static boolean retainAnnotatedMacroCalls;
    ...
    public static AST getAlgorithm(PcalCharReader charR, boolean fairAlgorithm, boolean onlyExpandUnannotatedMacros) throws ParseAlgorithmException {
        return getAlgorithm(charR, fairAlgorithm, onlyExpandUnannotatedMacros, false).getFirst();
    }

    public static Pair<AST, Vector> getAlgorithm(PcalCharReader charR, boolean fairAlgorithm, boolean onlyExpandUnannotatedMacros, boolean returnMacros) throws ParseAlgorithmException {
        try {
            retainAnnotatedMacroCalls = onlyExpandUnannotatedMacros;
            Init(charR);
            if (fairAlgorithm) {
                String nextToken = GetAlgToken();
                if (!nextToken.equals(PcalParams.BeginFairAlg2)) {
                    ParsingError("`" + PcalParams.BeginFairAlg + "' not followed by `" + PcalParams.BeginFairAlg2 + "'");
                }
            ...
            AnnotationFunction isAnnotation = Preprocessor::isAnnotation;
            PeekAtAlgTokenFunction peekAlgTokenFunction;

            Vector macros = new Vector<>();
            while (PeekAtAlgToken(1).equals("macro") || PeekAtAlgToken(1).equals("§")) {
                if (PeekAtAlgToken(1).equals("macro")) {
                    macros.addElement(GetMacro(macros));
                    System.out.printf("");
                } else if (PeekAtAlgToken(1).equals("§")) {
                    peekAlgTokenFunction = ParseAlgorithm::PeekAtAlgToken;
                    Pair<Integer, ArrayList<String>> ret = Preprocessor.calculateOffset(charReader, peekAlgTokenFunction, isAnnotation);
                    int offset = ret.getFirst();
                    if (charReader.vec.get(charReader.line + 1).toString().contains("macro")) {
                        Gobbler.consumeNext(offset);
                        AST.Macro macro = GetMacro(macros);
                        macro.annotations = ret.getSecond();
                        if (!macro.annotations.isEmpty()) {
                            macrosWithAnnotations.put(macro.name, macro.annotations);
                        }
            ...
            Pair<Integer, ArrayList<String>> ret = Preprocessor.calculateOffset(charReader, peekAlgTokenFunction, isAnnotation);
            int offset = ret.getFirst();
            boolean a = PeekAtAlgToken(offset + 1).equals("fair");
            boolean b = PeekAtAlgToken(offset + 2).equals("process");
            boolean c = PeekAtAlgToken(offset + 2).equals("+");
            boolean d = PeekAtAlgToken(offset + 3).equals("process");
            boolean e = PeekAtAlgToken(offset + 1).equals("process");
            if ((a && b) || (a && c && d) || e) {
                ...
                multiproc.procedures = procedures;
                multiproc.processes = new Vector();
                while (PeekAtAlgToken(offset + 1).equals("fair") || PeekAtAlgToken(offset + 2).equals("process")) {
                    if (offset > 0) {
                        Gobbler.consumeNext(offset);
                    } else {
                        PcalDebug.reportInfo("No annotations detected");
                    }
                    ...
                    proc.annotations = ret.getSecond();
                    multiproc.processes.addElement(proc);
                    if (PeekAtAlgToken(1).equals("§")) {
                        peekAlgTokenFunction = ParseAlgorithm::PeekAtAlgToken;
                        ret = Preprocessor.calculateOffset(charReader, peekAlgTokenFunction, isAnnotation);
                        offset = ret.getFirst();
                    } else {
                        ret = new Pair<>(0, new ArrayList<>());
                        offset = 0;
                    }
                }
                if (pSyntax) {
                ...
                while (i < multiproc.processes.size()) {
                    AST.Process proc = multiproc.processes.elementAt(i);
                ...
                i = 0;
                while (i < multiproc.procedures.size()) {
                    AST.Procedure prcd = multiproc.procedures.elementAt(i);
                ...
                if (!addedLabels.isEmpty()) {
                    if (!PcalParams.LabelFlag) { // TODO danger
                        AddedMessagesError();
                    }
                ...
                multiproc.setOrigin(new Region(multiprocBegin, GetLastLocationEnd()));
                return new Pair<>(multiproc, macros);
            } else {
                ...
                uniproc.setOrigin(new Region(uniprocBegin, GetLastLocationEnd()));
                return new Pair<>(uniproc, macros);
            }
            ...
            System.err.println("Error in algorithm: " + e.getMessage());
    ...
    private static void checkBody(Vector body) {
        ...
        for (int i = 0; i < whileStmt.unlabDo.size(); i++) {
            ...
            if (obj.getClass().equals(AST.LabelIfObj.getClass()) || obj.getClass().equals(AST.LabelEitherObj.getClass()) || obj.getClass().equals(AST.LabeledStmtObj.getClass())) {
                omitPC = false;
                return;
            }
        }
...
//       CheckLabeledStmtSeq(result.body) ;
    ...
    public static Vector<Object> GetVarDecls() throws ParseAlgorithmException
    /**********************************************************************
     ...
     **********************************************************************/
    {
        ...
        while (!(PeekAtAlgToken(1).equals("begin")
            ...
            if (PeekAtAlgToken(1).equals("end") && PeekAtAlgToken(2).equals(";")) {
                Gobbler.consumeNext(2); // consume "end ;"
                internalVariables = false;
            }
        }
        return result;
    }
    ...
    public static AST.VarDecl GetVarDecl() throws ParseAlgorithmException {
        ...
        if (PeekAtAlgToken(1).equals(Preprocessor.paragraphChar)
                && PeekAtAlgToken(2).equals("internal")
                && PeekAtAlgToken(3).equals("begin")) {
            internalVariables = true;
            Gobbler.consumeNext(3); // consume "internal begin"
        }
        pv.var = GetAlgToken();
        PCalLocation beginLoc = GetLastLocationStart();
        PCalLocation endLoc = GetLastLocationEnd();
        ...
        if (PeekAtAlgToken(1).equals("=") || PeekAtAlgToken(1).equals("\\in")) {
            ...
            Pair<TLAExpr, List<Pair<String, String>>> expOpt = GetExpr(true);
            pv.val = expOpt.getFirst();
            pv.isInternal = internalVariables;
            pv.annotations.addAll(expOpt.getSecond());
            endLoc = pv.val.getOrigin().getEnd();
            if (pv.val.tokens.isEmpty()) {
                ParsingError("Missing expression at ");
            }
        } else {
            hasDefaultInitialization = true;
    ...
    public static TLAExpr GetExpr() throws ParseAlgorithmException {
        return GetExpr(true).getFirst();
    }

    public static Pair<TLAExpr, List<Pair<String, String>>> GetExpr(boolean specialReturn) throws ParseAlgorithmException {
        if (LATsize != 0) {
            PcalDebug.ReportBug("ParseAlgorithm: GetExpr called after lookahead");
        }
        Pair<TLAExpr, List<Pair<String, String>>> expOpt;
        TLAExpr result;
        try {
            expOpt = Tokenize.TokenizeExpr(charReader);
            result = expOpt.getFirst();
            result.annotations.addAll(expOpt.getSecond());
        ...
        if (result.tokens != null && result.tokens.size() != 0) {
            PCalLocation begin = ((TLAToken) ((Vector) result.tokens.elementAt(0)).elementAt(0)).source.getBegin();
            ...
            PCalLocation end = ((TLAToken) lastLineOfTokens.elementAt(lastLineOfTokens.size() - 1)).source.getEnd();
            result.setOrigin(new Region(begin, end));
        } else {
            result.setOrigin(null);
        }
        return new Pair<>(result, expOpt.getSecond());
    }
    ...
    public static String GetLabel() throws ParseAlgorithmException {
        ...
        if (IsLabelNext()) {
            ...
            if (inGetMacro) {
                PcalDebug.reportInfo("      🏷️ Label '" + nextLabel + "' in a macro");
                if (!currentMacroContext.isBlank()) {
                    macrosWithLabels.add(currentMacroContext);
                }
                dangerousLabels.add(nextLabel);
     ...
     **********************************************************************/
    {
        ...
        while (!((pSyntax && (tok.equals("end") || tok.equals("else") || tok.equals("elsif") || tok.equals("or"))) || (cSyntax && tok.equals("}")))) {
            ...
            if (cSyntax && PeekAtAlgToken(1).equals("{")) {
                 ...
                 ************************************************************/
                AST stmt = GetStmt(); /* TODO: IMPORTANT */
                stmt.lbl = nextLabel;
    ...
    public static AST GetStmt() throws ParseAlgorithmException {
        ...
        if (PeekPastAlgToken(1).charAt(0) == '(') {
            if (nextTok.matches("receive[A-Za-z]+")) {
                return retainAnnotatedMacroCalls ? GetReceiveCall(true) : GetMacroCall();
            } else {
                return switch (nextTok) {
                    case "send", "broadcast" -> GetSendCall(nextTok);
                    case "receive" -> GetReceiveCall();
                    case "fail" -> GetFail();
                    case "maybeFail" -> GetMaybeFail();
                    default -> GetMacroCall();
                };
            }
        }
        return GetAssign();
    ...
    public static AST.LabelIf GetIf(int depth) throws ParseAlgorithmException {
        ...
        Pair<TLAExpr, List<Pair<String, String>>> expOpt = GetExpr(true);
        result.test = expOpt.getFirst();
        result.annotations.addAll(expOpt.getSecond());
        if (cSyntax) {
            GobbleThis(")");
        }
        if (result.test.tokens.isEmpty()) {
...
//     }
    ...
    public static AST.Macro GetMacro(Vector macros) throws ParseAlgorithmException {
        ...
        result.line = lastTokLine;
        result.name = GetAlgToken();
        currentMacroContext = result.name;
        GobbleThis("(");
        result.params = new Vector();
        boolean lookForComma = false;
        while (!PeekAtAlgToken(1).equals(")")) {
    ...
    public static AST.MacroCall GetMacroCall() throws ParseAlgorithmException {
        ...
        result.name = GetAlgToken();
        result.annotations = macrosWithAnnotations.get(result.name);
        if (result.annotations == null) {
            result.annotations = new ArrayList<String>();
        }
        PCalLocation beginLoc = GetLastLocationStart();
        result.col = lastTokCol;
        result.line = lastTokLine;
        ...
        while (moreArgs) {
            result.args.addElement(GetExpr());
            if (((TLAExpr) result.args.lastElement()).tokens.isEmpty()) {
                ParsingError("Empty argument in call statement at ");
            }
            if (!PeekAtAlgToken(1).equals(")")) {
                GobbleThis(",");
...
// PcalDebug.printVector(outAssigned, "out") ;
     ...
     **********************************************************************/
    {
        ...
        while (i < stmtseq.size()) {
            ...
            if (node.getClass().equals(AST.LabelIfObj.getClass())) {
                ...
                ;
                result = 1;
            } else if (node.getClass().equals(AST.GotoObj.getClass()) || node.getClass().equals(AST.CallGotoObj.getClass())) {
                result = 1;
            } else if (!(node.getClass().equals(AST.AssignObj.getClass()) || node.getClass().equals(AST.WhenObj.getClass()) || node.getClass().equals(AST.PrintSObj.getClass()) || node.getClass().equals(AST.AssertObj.getClass()) || node.getClass().equals(AST.SkipObj.getClass()) || node.getClass().equals(AST.MacroCallObj.getClass()) || node.getClass().equals(AST.ReceiveCallObj.getClass()) || node.getClass().equals(AST.SendCallObj.getClass()) || node.getClass().equals(AST.FailObj.getClass()) || node.getClass().equals(AST.MaybeFailObj.getClass()))) {
    ...
    public static void ObsoleteExpandMacrosInLabeledStmtSeq(Vector stmtseq, // of LabeledStmt
    ...
    public static void ExpandMacrosInStmtSeq(Vector stmtseq, Vector macros) throws ParseAlgorithmException {
        ...
        while (i < stmtseq.size()) {
            AST node = (AST) stmtseq.elementAt(i);
            if (node.getClass().equals(AST.LabelIfObj.getClass())) {
                ExpandMacrosInStmtSeq(((AST.LabelIf) node).unlabThen, macros);
                ExpandMacrosInStmtSeq(((AST.LabelIf) node).unlabElse, macros);
            } else if (node.getClass().equals(AST.LabelEitherObj.getClass())) {
            ...
            } else if (node.getClass().equals(AST.MacroCallObj.getClass())) {
                AST.MacroCall macroCall = (AST.MacroCall) node;
                macroExpandCount.computeIfPresent(macroCall.name, (k, v) -> v + 1);
                macroExpandCount.putIfAbsent(macroCall.name, 0);
                Vector expansion = ExpandMacroCall(macroCall, macros);
                stmtseq.remove(i);
    ...
    public static Vector ExpandMacroCall(AST.MacroCall call, Vector macros) throws ParseAlgorithmException {
        if (!call.annotations.isEmpty() && retainAnnotatedMacroCalls) {
            Vector result = new Vector();
            result.add(call);
            return result;
        }
        ...
        int labelContext = macroExpandCount.getOrDefault(call.name, 0);
        if (labelContext > 0) {
            macroDef.body = LabelTransformer.recursiveDescent(macroDef.body, "_" + call.name);
        }
        ...
        Vector<?> result = SubstituteInStmtSeq(macroDef.body, call.args, macroDef.params, call.line, call.col, macros, labelContext);
        /*
    ...
    public static Vector SubstituteInLabeledStmtSeq(Vector stmts, Vector args, Vector params, Vector macros, int labelContext) throws ParseAlgorithmException // of String
    {
        Vector result = new Vector();
        int i = 0;
        while (i < stmts.size()) {
            result.addElement(SubstituteInLabeledStmt((AST.LabeledStmt) stmts.elementAt(i), args, params, macros, labelContext));
            i = i + 1;
        }
        return result;
    }
    ...
    public static AST.LabeledStmt SubstituteInLabeledStmt(AST.LabeledStmt stmt, Vector args,   // of TLAExpr
                                                          Vector params, Vector macros, int labelContext) throws ParseAlgorithmException // of String
    {
        AST.LabeledStmt result = new AST.LabeledStmt();
        result.label = stmt.label;
        result.stmts = SubstituteInStmtSeq(stmt.stmts, args, params, -1, 0, macros, labelContext);
    ...
    public static Vector SubstituteInStmtSeq(Vector stmts, // of AST
                                             ...
                                             int macroLine, int macroCol, Vector macros,
                                             int labelContext) throws ParseAlgorithmException
    /*********************************************************************
     ...
     *********************************************************************/
    {
        ...
        int i = 0;
        while (i < stmts.size()) {
            AST node = ((AST) stmts.elementAt(i)).deepCopy();
            if (node instanceof AST.MacroCall) {
                AST.MacroCall macroCall = (AST.MacroCall) node;
                if (retainAnnotatedMacroCalls) {
                    result.addElement(macroCall);
                } else {
                    result.addAll(ExpandMacroCall(macroCall, macros));
                }
            } else {
                result.addElement(SubstituteInStmt(node, args, params, macroLine, macroCol, macros, labelContext));
            }
            i = i + 1;
        }
        return result;
        ...
        try {
            if (stmt.getClass().equals(AST.AssignObj.getClass())) {
                ...
                while (i < tstmt.ass.size()) {
                    result.ass.addElement(SubstituteInSingleAssign((AST.SingleAssign) tstmt.ass.elementAt(i), args, params, macroLine, macroCol));
                    i = i + 1;
                }
                return result;
            }
            ...
            if (stmt.getClass().equals(AST.IfObj.getClass())) {
                ...
                result.Then = SubstituteInStmtSeq(tstmt.Then, args, params, macroLine, macroCol, macros, labelContext);

                result.Else = SubstituteInStmtSeq(tstmt.Else, args, params, macroLine, macroCol, macros, labelContext);
            ...
            if (stmt.getClass().equals(AST.EitherObj.getClass())) {
                ...
                while (i < tstmt.ors.size()) {
                    result.ors.addElement(SubstituteInStmtSeq((Vector) tstmt.ors.elementAt(i), args, params, macroLine, macroCol, macros, labelContext));
                    i = i + 1;
                }
                return result;
            }
            ...
            if (stmt.getClass().equals(AST.WithObj.getClass())) {
                ...
                result.Do = SubstituteInStmtSeq(tstmt.Do, args, params, macroLine, macroCol, macros, labelContext);
                return result;
            }
            ...
            if (stmt.getClass().equals(AST.MaybeFailObj.getClass())) {
                ...
                while (i < tstmt.either.ors.size()) {
                    result.either.ors.addElement(SubstituteInStmtSeq((Vector) tstmt.either.ors.elementAt(i), args, params, macroLine, macroCol, macros, labelContext));
                    i = i + 1;
                }
                return result;
            }
            ...
            if (stmt.getClass().equals(AST.WhileObj.getClass())) {
                ...
                result.test = tstmt.test.cloneAndNormalize();
                result.test.substituteForAll(args, params);
                result.unlabDo = SubstituteInStmtSeq(tstmt.unlabDo, args, params, macroLine, macroCol, macros, labelContext);
                result.labDo = SubstituteInLabeledStmtSeq(tstmt.labDo, args, params, macros, labelContext);
            ...
            if (stmt.getClass().equals(AST.LabelIfObj.getClass())) {
                ...
                result.test = tstmt.test.cloneAndNormalize();
                result.test.substituteForAll(args, params);
                result.unlabThen = SubstituteInStmtSeq(tstmt.unlabThen, args, params, macroLine, macroCol, macros, labelContext);
                result.labThen = SubstituteInLabeledStmtSeq(tstmt.labThen, args, params, macros, labelContext);
                result.unlabElse = SubstituteInStmtSeq(tstmt.unlabElse, args, params, macroLine, macroCol, macros, labelContext);
                result.labElse = SubstituteInLabeledStmtSeq(tstmt.labElse, args, params, macros, labelContext);
            ...
            if (stmt.getClass().equals(AST.LabelEitherObj.getClass())) {
                ...
                while (i < tstmt.clauses.size()) {
                    ...
                    newClause.labOr = SubstituteInLabeledStmtSeq(oldClause.labOr, args, params, macros, labelContext);
                    newClause.unlabOr = SubstituteInStmtSeq(oldClause.unlabOr, args, params, macroLine, macroCol, macros, labelContext);
    ...
    public static AST SubstituteInSingleAssign(AST.SingleAssign assgn, Vector args,   // of TLAExpr
                                               Vector params, // of String
                                               int macroLine, int macroCol) throws ParseAlgorithmException
    /*********************************************************************
...
//      { return  }
    ...
    public static void MustGobbleThis(final String str) throws ParseAlgorithmException {
        MustGobbleThis(str, false);
    }

    public static String MustGobbleThis(final String str, boolean useRegex, boolean ret) throws ParseAlgorithmException {
        final String tok;
        try {
            tok = GetAlgToken();
        } catch (ParseAlgorithmException e) {
        ...
        boolean unEquals = useRegex ? !Pattern.compile(str).matcher(tok).find() : !tok.equals(str);
        if (unEquals) {
            ParsingError("Expected \"" + str + "\" but found \"" + tok + "\"");
        }
        return ret ? tok : null;
    ...
    public static void MustGobbleThis(final String str, boolean useRegex) throws ParseAlgorithmException {
        final String tok;
        try {
            tok = GetAlgToken();
        } catch (ParseAlgorithmException e) {
    ...
    public static String[] LAT = new String[40];
    ...
    public static int[] curTokCol = new int[40];
    public static int[] curTokLine = new int[40];
    /**********************************************************************
    ...
    public static String SoftPeekAtAlgToken(int tokNum) throws ParseAlgorithmException {
        String[] LatCopy = new String[LAT.length];
        int LatSizeCopy = LATsize;
        System.arraycopy(LAT, 0, LatCopy, 0, LAT.length);
        while ((LatSizeCopy < tokNum)) {
            ...
            try {
                LatCopy[LatSizeCopy] = Tokenize.GetAlgorithmToken(charReader);
            } catch (TokenizerException e) {
                throw new ParseAlgorithmException(e.getMessage());
            }
            LatSizeCopy = LatSizeCopy + 1;
        }
        return LatCopy[tokNum - 1];
    }
        ...
        while (notDone && line < inp.size()) {
            ...
            while (notDone && col < oldLine.length()) {
                 ...
                 ********************************************************/
                    ...
                    {
                        if ((inChar == '\\') && (col < oldLine.length() - 1) && (oldLine.charAt(col + 1) == '*')) { // End-of-line comment
                            outChar = ' ';
                            col = oldLine.length();
                        } else if (inChar == '"') {
                            inString = true;
...
// printArray(argsArray);
     ...
     * @throws ParseAlgorithmException
     */
    public static void FindToken(String token, Vector inputVec, IntPair curLoc, String errorMsg  // The error message to be reported if not found.
    ) throws ParseAlgorithmException {
        boolean found = false;
        ...
        while (curLoc.one < inputVec.size()) {
            ...
            while (curLoc.two < curLine.length()) {
                ...
                if (curLoc.two < curLine.length()) {
                    ...
                    } else if ((c == '(') && (curLoc.two + 1 < curLine.length()) && (curLine.charAt(curLoc.two + 1) == '*')) {
                        ParseAlgorithm.gotoEndOfComment(inputVec, curLoc);
                        curLine = (String) inputVec.elementAt(curLoc.one);
                    } else if ((c == '\\') && (curLoc.two + 1 < curLine.length()) && (curLine.charAt(curLoc.two + 1) == '*')) {
                        // if (replace) {
...
//      }
    ...
    private static AST GetSendCall(String nextTok) throws ParseAlgorithmException {
        ...
        if (PCalErlangConstants.SEND_ALIAS.contains(nextTok)) {
            result.type = PCalErlangConstants.SendType.SEND;
        } else if (PCalErlangConstants.BROADCAST_ALIAS.contains(nextTok)) {
    ...
    private static AST GetReceiveCall() throws ParseAlgorithmException {
        return GetReceiveCall(false);
    }

    private static AST GetReceiveCall(boolean useRegex) throws ParseAlgorithmException {
        AST.ReceiveCall result = new AST.ReceiveCall();
        PCalLocation beginLoc = GetLastLocationStart();
        String match = useRegex ? "receive[a-zA-Z]+" : "receive";
        MustGobbleThis(match, useRegex);
        MustGobbleThis("(");
        result.col = lastTokCol;
        result.line = lastTokLine;
        ...
        if (PeekAtAlgToken(1).equals(",")) {
            GobbleThis(",");
            result.afterVarname = GetAlgToken();
        }
