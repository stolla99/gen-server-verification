...
//MappingObject.printMappingVector(mappingVector);
    ...
    {
        mp = true;
        GenVarsAndDefs(ast.decls, ast.procedures, ast.processes, ast.defs);
        GenProcSet();
        GenInit(ast.decls, ast.procedures, ast.processes);
        for (int i = 0; i < ast.procedures.size(); i++)
            GenProcedure((AST.Procedure) ast.procedures.elementAt(i), "");
        for (int i = 0; i < ast.processes.size(); i++)
            GenProcess((AST.Process) ast.processes.elementAt(i), "");
        GenNext();
        GenSpec();
        GenTermination();
    }
...
//        tlacode.addElement("");
    ...
    private void GenLabeledStmt(AST.LabeledStmt ast, String context) throws PcalTLAGenException
    {
         ...
         */
        Vector unc = c.Unchanged(999 - wrapColumn - col - "/\\ UNCHANGED << ".length());
        if (c.NumUnchanged() > 1)
        {
            sb = new StringBuffer(NSpaces(col));
            sb.append("/\\ UNCHANGED << ");
      ...
      throws PcalTLAGenException
    {
        ...
        *******************************************************************/
        if (globals != null)
            ...
            {
                VarDecl decl = (VarDecl) globals.elementAt(i);
        ...
        if (processes != null)
            for (int i = 0; i < processes.size(); i++)
            {
                ...
                if (proc.decls != null)
                    for (int p = 0; p < proc.decls.size(); p++)
                    {
                        VarDecl decl = (VarDecl) proc.decls.elementAt(p);
                        lVars.addElement(decl.var);
...
//                tlacodeNextLine = tlacodeNextLine + ", ";
            ...
            VarDecl vblDecl = (VarDecl) gVarsSource.elementAt(i);
